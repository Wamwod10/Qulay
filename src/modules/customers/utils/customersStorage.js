...
const FOLLOW_UPS_KEY = "universal_erp_customer_followups";

const canUseStorage = () => typeof window !== "undefined" && window.localStorage;

const createId = (prefix = "cus") =>
...
export const normalizeCustomer = (customer = {}) => ({
  ...
  segment: CUSTOMER_SEGMENTS.includes(String(customer.segment || "").toUpperCase())
    ? String(customer.segment).toUpperCase()
    : "REGULAR",
  source: CUSTOMER_SOURCES.includes(String(customer.source || "").toUpperCase())
    ? String(customer.source).toUpperCase()
    : "OTHER",
...
const readFollowUps = () => {
  ...
  try {
    const storedValue = window.localStorage.getItem(FOLLOW_UPS_KEY);
    ...
    const followUps = JSON.parse(storedValue);
...
export const normalizeCustomerFollowUp = (followUp = {}) => ({
  ...
  type: ["CALL", "MEETING", "PAYMENT", "SALES", "OTHER"].includes(followUp.type)
    ? followUp.type
    : "OTHER",
  status: ["OPEN", "DONE", "CANCELLED"].includes(followUp.status)
    ? followUp.status
    : "OPEN",
...
export const saveCustomerFollowUps = (followUps) => {
  ...
  window.localStorage.setItem(
    FOLLOW_UPS_KEY,
    JSON.stringify(followUps.map(normalizeCustomerFollowUp)),
  );
  window.dispatchEvent(new Event("customers:changed"));
