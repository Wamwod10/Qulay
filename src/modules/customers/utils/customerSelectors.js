...
import {
  getCustomerDebt as getFinanceCustomerDebt,
...
const activeCustomerSales = (customerId) =>
  getStoredSales().filter((sale) => sale.customerId === customerId && !isCancelledSale(sale));

export const getCustomerSales = (customerId) =>
  activeCustomerSales(customerId).sort((a, b) => getSaleDateValue(b) - getSaleDateValue(a));

export const getCustomerOrdersCount = (customerId) => getCustomerSales(customerId).length;

export const getCustomerSalesTotal = (customerId) =>
  roundMoney(
    getCustomerSales(customerId).reduce(
      (total, sale) => total + toMoney(sale.netTotal ?? sale.total),
      0,
    ),
  );
...
export const getCustomerFollowUps = (customerId) =>
  getStoredCustomerFollowUps()
...
export const getOpenCustomerFollowUps = (customerId) =>
  getCustomerFollowUps(customerId).filter((followUp) => followUp.status === "OPEN");

export const getNextCustomerFollowUp = (customerId) =>
  [...getOpenCustomerFollowUps(customerId)].sort((a, b) => {
...
export const getCustomerPayments = (customerId) =>
  getFinanceTransactions({ customerId }).filter((transaction) =>
    ["SALE_PAYMENT", "CUSTOMER_PAYMENT", "AGENT_COLLECTION", "REFUND"].includes(
      transaction.sourceType,
    ),
  );

export const getCustomerCollectedAmount = (customerId) =>
  roundMoney(
...
export const canCustomerUseDebt = ({ customerId, additionalDebt = 0 }) => {
...
export const calculateCustomerScore = (customerId) => {
  ...
  const daysSinceLastSale = lastSaleDate
    ? Math.max(Math.floor((Date.now() - lastSaleDate.getTime()) / DAY_MS), 0)
    : null;
  ...
  return {
    value,
    label:
      value >= 80 ? "Juda yaxshi" : value >= 60 ? "Yaxshi" : value >= 40 ? "O'rtacha" : "Riskli",
...
export const getCustomerDeleteSafety = (customerId) => {
  ...
  const financeCount = getStoredFinanceTransactions().filter(
  ...
  if (salesCount) {
    blockingReasons.push(`${salesCount} ta savdo mavjud`);
  }

  if (financeCount) {
    blockingReasons.push(`${financeCount} ta finance transaction mavjud`);
  }
...
export const buildCustomerListRows = () => {
  ...
  return getStoredCustomers().map((customer) => {
    ...
    return {
      ...
      agent,
      agentName: agent?.name || (customer.agentId ? "Agent topilmadi" : "-"),
...
export const buildCustomerReport = () => {
  ...
  rows.forEach((customer) => {
    ...
    const current = agentMap.get(key) || {
      id: key,
      name: customer.agentName || "Agent tanlanmagan",
      customers: 0,
      sales: 0,
      debt: 0,
    };
  ...
  return {
    ...
    riskyDebtCustomers: rows
      .filter((customer) => customer.debtAmount > 0 || customer.score.value < 40)
      .sort((a, b) => b.debtAmount - a.debtAmount || a.score.value - b.score.value)
      .slice(0, 5),
